...
describe('SessionSidePanel - persisted event session ownership', () => {
  ...
  it('ignores and evicts a persisted event owned by a different session', async () => {
    ...
    // The stale event must NOT render inside the newly selected session; we fall
    // back to the session root instead.
    ...
    // ...and the stale param is cleared from the URL.
    await waitFor(() =>
      expect(mockNuqs.setSessionPanelEvent).toHaveBeenCalledWith(null),
    );
  });
});
