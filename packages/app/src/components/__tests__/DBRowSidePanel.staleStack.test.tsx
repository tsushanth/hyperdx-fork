...
describe('DBRowSidePanelInner — stale stack handling (HDX-3942 "Stale Stack Remains")', () => {
  ...
  it('clears a stale trail left in the URL when a different root row mounts', () => {
    ...
    // The row the user actually opened wins over the stale leaf...
    expect(lastResolvedRowId()).toBe('new-root');
    // ...and the stale params are cleared from the URL.
  ...
  it('clears the trail when the root row changes while the panel stays mounted', () => {
    seedParam('sidePanelSourceStack', [TRACE_FRAME]);
    seedParam('sidePanelNavStack', []);
    ...
    // clicked another row in the same table); the trail is stale and dropped.
    rerender(
      <MantineProvider>
        <DBRowSidePanelInner
          source={ROOT_SOURCE as any}
  ...
  it('does not treat a trail with no recorded root as stale (safe fallback)', () => {
    // Legacy / hand-crafted URL: stacks present but no stackRoot token. We must
    // not clobber a trail we cannot prove is stale.
    seedParam('sidePanelSourceStack', [TRACE_FRAME]);
    seedParam('sidePanelNavStack', []);
    // sidePanelStackRoot intentionally unseeded (null).
